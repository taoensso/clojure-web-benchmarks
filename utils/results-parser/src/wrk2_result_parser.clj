...
(def ordered-headers 
  (doall (map #(% tb-headers) [:server :port :threads :conns :qps :mlat :n4lat :err-rate :errors :total])))
...
(defn parse-header [[s r]]
  (let [[[_ port] s] (try-match #"\:(\d+)\/" s)
    ...
    [s (assoc r (tb-headers :port) port  (tb-headers :threads) threads   (tb-headers :conns) conns)]))
...
(defn parse-lats [[s r]]
  (let [[[_ mlat] s] (try-match #"(\d+[^\s]+)" s)
   ...
   ;(println "parse-lats"  {:mlat  mlat :nnnnlat nnnnlat} )
    [ s (assoc r (tb-headers :mlat) (trim-num (format-time mlat))
...
(defn parse-errors [[s r]]
  (let [ [ss s] (try-match #"\d+" s)
              ...
              total (+ ss errors)]
    [s (assoc r (tb-headers :errors) errors (tb-headers :total) total (tb-headers :err-rate) (trim-num  (/ (* errors 100) (float total))))]))

(defn parse-qps [[s r]]
 (let [[qps s] (try-match #"\d+" s)]
   [s (assoc r (tb-headers :qps) (Integer/parseInt qps))]))
...
(defn parse-one [s]
  (-> [s {} ]
         ...
         (parse-qps)
         (second)
         (#(assoc % (tb-headers :server) (port-servers (% (tb-headers :port)))))))

(defn parse-file 
  "parse a stripped wrk2 results file"
  [f]
  ;(println "file " f)
...
(defn save-pngs! [r f title keep-alive?]
  (let [y-label (if keep-alive? "(wrk2 Keep-Alive:On)" "(wrk2 Keep-Alive:Off)")
      ...
      (ic/with-data (ic/to-dataset r)
		    (ic/save (ics/bar-chart server  (tb-headers :qps)
		                            :group-by "Conns" :legend true :vertical false :title title :y-label (str 
		                                                                                                   (tb-headers :qps) y-label )) (str f "-qps.png") :width 800 :height 600)
		    (ic/save (ics/bar-chart server  (tb-headers :mlat)
		                            :group-by "Conns" :legend true :vertical false :title title :y-label (str 
		                                                                                                   (tb-headers :mlat) y-label )) (str f "-mlat.png") :width 800 :height 600)
		    (ic/save (ics/bar-chart server  (tb-headers :n4lat)
		                            :group-by "Conns" :legend true :vertical false :title title :y-label (str 
		                                                                                                   (tb-headers :n4lat) y-label )) (str f "-n4lat.png") :width 800 :height 600)
		    (ic/save (ics/bar-chart server  (tb-headers :err-rate)
		                            :group-by "Conns" :legend true :vertical false :title title :y-label (str 
		                                                                                                   (tb-headers :err-rate) y-label )) (str f "-errs.png") :width 800 :height 600)
		    ))
)
